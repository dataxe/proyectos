...
$precio_venta_t1 = $productos['vlr_total_venta'] / $productos['unidades'];

$unidades_t0 = $unidades_vendidas * $productos['unidades'];
$unidades_t1 = $unidades_vendidas / $productos['unidades'];

$vlr_total_venta_t0 = $productos['precio_venta'] * $temporal['unidades_vendidas'];
$vlr_total_venta_t1 = $productos['vlr_total_venta'];

$detalles_0 = $productos['detalles'];
$detalles_1 = 'UND';

if ($tipo_venta == '0') {
$actualizar_sql = sprintf("UPDATE temporal SET tipo_venta=%s, unidades_vendidas=%s, precio_venta=%s, vlr_total_venta=%s WHERE cod_temporal=%s",
...
$actualizar_sql = sprintf("UPDATE temporal SET tipo_venta=%s, precio_venta=%s, vlr_total_venta=%s WHERE cod_temporal=%s",
                       envio_valores_tipo_sql($tipo_venta, "text"),
