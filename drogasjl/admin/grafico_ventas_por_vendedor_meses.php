...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'VENTAS POR MES - <?php echo $vendedor?>'
        },
        subtitle: {
        ...
        xAxis: {
            categories: [
<?php
$sql = "SELECT SUM(vlr_total_venta) AS vlr_total_venta, fecha_mes FROM ventas WHERE vendedor = '$vendedor' GROUP BY fecha_mes ORDER BY fecha ASC";
$result = mysql_query($sql, $conectar) or die(mysql_error());
while ($registros = mysql_fetch_array($result)) {
?>
'<?php echo $registros["fecha_mes"] ?>',
<?php
}
?>
            ]
        ...
        series: [{
            ...
            data: [
<?php
$sql = "SELECT SUM(vlr_total_venta) AS vlr_total_venta FROM ventas WHERE vendedor = '$vendedor' GROUP BY fecha_mes ORDER BY fecha ASC";
$result = mysql_query($sql, $conectar) or die(mysql_error());
while ($registros = mysql_fetch_array($result)) {
?>
