...
$fecha_concatenada = addslashes($_POST['fecha']);
$frag_fecha = explode('-', $fecha_concatenada);

$fecha = addslashes($frag_fecha[0]);
$vendedor = addslashes($frag_fecha[1]);
...
FROM ventas WHERE fecha_anyo = '$fecha' AND vendedor = '$vendedor' ORDER BY cod_ventas DESC";
...
WHERE (tipo_pago ='1' OR tipo_pago ='') AND fecha_anyo = '$fecha' AND vendedor = '$vendedor'";
...
sum(precio_costo*unidades_vendidas) As vlr_total_compra, Sum(((precio_venta/1.16)*(iva/100))*unidades_vendidas) As sum_iva FROM ventas 
WHERE fecha_anyo = '$fecha' AND tipo_pago ='2' AND vendedor = '$vendedor'";
...
<?php 
$sql_consulta1="SELECT fecha_anyo, vendedor, fecha FROM ventas GROUP BY vendedor, fecha ORDER BY fecha DESC";
...
?>
<center>
<fieldset><legend><font color='yellow' size='+3'>VENTAS TOTALES DE <?php echo strtoupper($vendedor) ?> DIA: <?php echo $fecha ?></font></legend>
<table width='40%'>
<tr>
