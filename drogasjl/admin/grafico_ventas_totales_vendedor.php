...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'VENTAS TOTALES VENDEDORES<br><?php echo $empresa ?>'
        },
        tooltip: {
        ...
        series: [{
            ...
            data: [
            <?php
            $sql = "SELECT SUM(vlr_total_venta) AS vlr_total_venta, vendedor FROM ventas GROUP BY vendedor ORDER BY vlr_total_venta DESC";
            $result = mysql_query($sql, $conectar) or die(mysql_error());
           while ($registros = mysql_fetch_array($result)) {
           ?>
['<?php echo strtoupper($registros["vendedor"]).'<br> $ '.number_format($registros["vlr_total_venta"], 0, ",", "."); ?>', <?php echo $registros["vlr_total_venta"]; ?>],
           <?php
            }
            ?>
            ]
