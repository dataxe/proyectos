...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'PRODUCTOS MAS VENDIDOS (PRECIO VENTA)'
        },
        subtitle: {
        ...
        xAxis: {
            categories: [
<?php
$sql = "SELECT SUM(vlr_total_venta) AS vlr_total_venta, cod_productos, nombre_productos FROM ventas GROUP BY cod_productos ORDER BY vlr_total_venta DESC LIMIT 0,100";
...
?>
        ...
        series: [{
            ...
            data: [
<?php
$sql = "SELECT SUM(vlr_total_venta) AS vlr_total_venta FROM ventas GROUP BY cod_productos ORDER BY vlr_total_venta DESC LIMIT 0,100";
$result = mysql_query($sql, $conectar) or die(mysql_error());
while ($registros = mysql_fetch_array($result)) {
?>
