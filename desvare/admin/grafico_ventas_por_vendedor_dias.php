...
$(function () {
    $('#container').highcharts({
        ...
        xAxis: {
            categories: [
<?php
$sql = "SELECT SUM(vlr_total_venta) AS vlr_total_venta, fecha_anyo FROM ventas WHERE vendedor = '$vendedor' GROUP BY fecha ORDER BY fecha ASC";
...
?>
        ...
        series: [{
            ...
            data: [
<?php
$sql = "SELECT SUM(vlr_total_venta) AS vlr_total_venta FROM ventas WHERE vendedor = '$vendedor' GROUP BY fecha ORDER BY fecha ASC";
$result = mysql_query($sql, $conectar) or die(mysql_error());
while ($registros = mysql_fetch_array($result)) {
?>
