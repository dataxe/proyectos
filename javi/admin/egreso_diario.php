...
$fecha = mysql_escape_string($_GET['fecha']);
$mostrar_datos_sql = "SELECT * FROM egresos WHERE fecha_invert = '$fecha' order by fecha_invert DESC";
$consulta = mysql_query($mostrar_datos_sql, $conectar) or die(mysql_error());
$datos = mysql_fetch_assoc($consulta);

$datos_sql = "SELECT SUM(costo) AS costo FROM egresos WHERE fecha_invert = '$fecha' order by fecha_invert DESC";
...
<?php $sql_consulta="SELECT DISTINCT fecha_anyo, fecha_invert FROM egresos ORDER BY fecha_invert DESC";
$resultado = mysql_query($sql_consulta, $conectar) or die(mysql_error());
while ($contenedor=mysql_fetch_array($resultado)) {
$fecha_segundo = strtotime($contenedor['fecha_invert']);
$fecha_salida = date("d/m/Y", $fecha_segundo);
?>
<option style="font-size:20px" value="<?php echo $contenedor['fecha_invert'] ?>"><?php echo $fecha_salida ?></option>
<?php }?></select></td>
<tr valign="baseline">
...
<?php
if (isset($_GET['fecha'])) {
$fecha_segundo2 = strtotime($fecha);
$fecha_salida2 = date("d/m/Y", $fecha_segundo2);

echo "<font color ='yellow' size='+3'>RESULTADOS PARA: ".$fecha_salida2."</font>";
...
<?php
$fecha_segundo3 = strtotime($datos['fecha_invert']);
$fecha_salida3 = date("d/m/Y", $fecha_segundo3);
?>
<td align="center"><?php echo $fecha_salida3; ?></td>
