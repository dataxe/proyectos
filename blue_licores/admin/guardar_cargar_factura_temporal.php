...
$calc_dto1 = ($datos['precio_costo'] * $unidades_total) - (($datos['precio_costo'] * $unidades_total) * ($datos['dto1']/100));
...
$descuento = ($datos['precio_costo'] * $cajas) - $calc_precio_compra_descuento;
...
valor_iva = '$valor_iva', precio_compra_con_descuento = '$precio_compra_con_descuento', descuento = '$descuento' 
...
$valor_iva = $calc_precio_compra_descuento * ($datos['iva']/100);
$precio_costo = (($calc_dto1 - ($calc_dto1 * ($datos['dto2']/100))) + $valor_iva) / $unidades_total;
$descuento = ($datos['precio_costo'] * $datos['cajas']) - $calc_precio_compra_descuento;
$precio_compra_con_descuento = ($calc_precio_compra_descuento + $valor_iva);
$precio_costo_unit = (($calc_dto1 - ($calc_dto1 * ($datos['dto2']/100)))) / $unidades_total;
$vlr_total_compra = $precio_costo_unit * $unidades_total;
//$vlr_total_venta = $precio_compra_con_descuento + ($precio_compra_con_descuento * ($datos['ptj_ganancia']/100));
...
//PARA PRECIO COMPRA
if ($campo == 'precio_compra' && $detalles <> 'UND') {
...
mysql_query("UPDATE cargar_factura_temporal SET precio_compra = '$precio_compra' 
WHERE cod_cargar_factura_temporal = '$cod_cargar_factura_temporal'", $conectar);
}
if ($campo == 'precio_compra' && $detalles == 'UND') {
$precio_compra = $valor_intro;
...
$unidades_total = ($datos['cajas'] * $datos['unidades']);
$calc_dto1 = ($precio_costo * $datos['cajas']) - (($precio_costo * $datos['cajas']) * ($datos['dto1']/100));
$calc_precio_compra_descuento = (($calc_dto1 - ($calc_dto1 * ($datos['dto2']/100))));
$valor_iva = $calc_precio_compra_descuento * ($datos['iva']/100);
$descuento = ($precio_costo * $datos['cajas']) - $calc_precio_compra_descuento;
$precio_compra_con_descuento = ($calc_precio_compra_descuento + $valor_iva) * $unidades_total;
$precio_costo_unit = (($calc_dto1 - ($calc_dto1 * ($datos['dto2']/100)))) / $unidades_total;
$vlr_total_compra = $precio_costo_unit * $unidades_total;
$descripcion = $precio_costo.' - '.$datos['vlr_total_venta'];

mysql_query("UPDATE cargar_factura_temporal SET precio_costo = '$precio_costo', precio_compra_con_descuento = '$precio_compra_con_descuento' 
WHERE cod_cargar_factura_temporal = '$cod_cargar_factura_temporal'", $conectar);
}
//PARA PRECIO VENTA TOTAL
if ($campo == 'precio_venta') {
$precio_venta = $valor_intro;
$descripcion = $datos['precio_costo'].' - '.$datos['vlr_total_venta'];

mysql_query("UPDATE cargar_factura_temporal SET precio_venta = '$precio_venta' 
WHERE cod_cargar_factura_temporal = '$cod_cargar_factura_temporal'", $conectar);
}
...
$precio_venta = $vlr_total_venta / $datos['unidades'];
$descripcion = $datos['precio_costo'].' - '.$vlr_total_venta;

mysql_query("UPDATE cargar_factura_temporal SET vlr_total_venta = '$vlr_total_venta' 
WHERE cod_cargar_factura_temporal = '$cod_cargar_factura_temporal'", $conectar);
}
//PARA PORCENTAJE GANANCIA
if ($campo == 'ptj_ganancia' && $detalles <> 'UND') {
...
if ($campo == 'ptj_ganancia' && $detalles == 'UND') {
$ptj_ganancia = $valor_intro;
$vlr_total_venta = $datos['vlr_total_venta'] + ($datos['vlr_total_venta'] * ($ptj_ganancia/100));
$precio_venta = $vlr_total_venta / $datos['unidades'];
...
$dto1 = $valor_intro;
$unidades_total = ($datos['cajas'] * $datos['unidades']);
$calc_dto1 = ($datos['precio_costo'] * $datos['cajas']) - (($datos['precio_costo'] * $datos['cajas']) * ($dto1/100));
...
$dto2 = $valor_intro;
$unidades_total = ($datos['cajas'] * $datos['unidades']);
$calc_dto1 = ($datos['precio_costo'] * $datos['cajas']) - (($datos['precio_costo'] * $datos['cajas']) * ($datos['dto1']/100));
$calc_precio_compra_descuento = (($calc_dto1 - ($calc_dto1 * ($dto2/100))));
$valor_iva = $calc_precio_compra_descuento * ($datos['iva']/100);
$precio_costo = (($calc_dto1 - ($calc_dto1 * ($dto2/100))) + $valor_iva) / $unidades_total;
$descuento = ($datos['precio_costo'] * $datos['cajas']) - $calc_precio_compra_descuento;
