...
$dato_fecha = explode('/', $_POST['fechas_vencimiento']);
$dia = $dato_fecha[0];
$mes = $dato_fecha[1];
$anyo = $dato_fecha[2];
...
echo '<META HTTP-EQUIV="REFRESH" CONTENT="7; ../admin/cargar_inventario.php">';
echo "</head>";
}
...
$actualizar_sql1 = sprintf("UPDATE productos SET cod_productos_var=%s, nombre_productos=%s, cod_marcas=%s, cod_proveedores=%s, cod_nomenclatura=%s, 
   cod_tipo=%s, cod_original=%s, codificacion=%s, unidades_faltantes=%s, unidades_vendidas=%s, precio_compra=%s, precio_costo=%s, precio_venta=%s, 
   ...
   fechas_dia=%s, fechas_mes=%s, fechas_anyo=%s, fechas_hora=%s, fechas_vencimiento=%s, fechas_vencimiento_seg=%s, vendedor=%s, porcentaje_vendedor=%s 
   WHERE cod_productos=%s",
envio_valores_tipo_sql($_POST['cod_productos_var'], "text"),
...
envio_valores_tipo_sql($fechas_dia, "text"),
envio_valores_tipo_sql($fechas_mes, "text"),
envio_valores_tipo_sql($fechas_anyo, "text"),
envio_valores_tipo_sql($fechas_hora, "text"),
envio_valores_tipo_sql($_POST['fechas_vencimiento'], "text"),
envio_valores_tipo_sql($fechas_vencimiento_seg, "text"),
envio_valores_tipo_sql($cuenta_actual, "text"),
envio_valores_tipo_sql($_POST['porcentaje_vendedor'], "text"),
envio_valores_tipo_sql($_GET['cod_productos'], "text"));

$resultado_actualizacion1 = mysql_query($actualizar_sql1, $conectar) or die(mysql_error());
echo '<META HTTP-EQUIV="REFRESH" CONTENT="0.2; ../admin/cargar_inventario.php">';

} elseif ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "formulario_de_actualizacion") && ($unidades == $datos['unidades_faltantes'])) {
$actualizar_sql1 = sprintf("UPDATE productos SET cod_productos_var=%s, nombre_productos=%s, cod_marcas=%s, cod_proveedores=%s, cod_nomenclatura=%s, 
   cod_tipo=%s, cod_original=%s, codificacion=%s, precio_compra=%s, precio_costo=%s, precio_venta=%s, utilidad=%s, total_utilidad=%s, total_mercancia=%s, total_venta=%s, 
   gasto=%s, detalles=%s, descripcion=%s, numero_factura=%s, cod_paises=%s, fechas_dia=%s, fechas_mes=%s, fechas_anyo=%s, fechas_hora=%s, fechas_vencimiento=%s, 
   fechas_vencimiento_seg=%s, vendedor=%s, porcentaje_vendedor=%s  WHERE cod_productos=%s",
envio_valores_tipo_sql($_POST['cod_productos_var'], "text"),
envio_valores_tipo_sql($_POST['nombre_productos'], "text"),
envio_valores_tipo_sql($_POST['cod_marcas'], "text"),
envio_valores_tipo_sql($_POST['cod_proveedores'], "text"),
...
<td><input type="text" name="cod_productos_var" value="<?php echo $datos['cod_productos_var']; ?>" size="70" required autofocus></td>
</tr>
...
<input type="hidden" name="codificacion" value="<?php echo $datos['codificacion']; ?>" size="70"></td>
<!--</tr>-->
<tr valign="baseline">
...
<tr valign="baseline">
<td nowrap align="left">ESTANTE:</td>
<td>
...
<td><input type="text" name="unidades" id="foco" value="<?php echo $datos['unidades_faltantes']; ?>" size="70" required autofocus></td>
</tr>
<tr valign="baseline">
<td nowrap align="left">PRECIO COMPRA:</td>
<td><input type="text" name="precio_compra" value="<?php echo $datos['precio_costo']; ?>" size="70" required autofocus></td>
</tr>
<tr valign="baseline">
...
<!--<tr valign="baseline">envio_valores_tipo_sql($_POST['fechas_vencimiento'], "text"),
...
<tr valign="baseline">
<td nowrap align="left">FECHA VENCIMIENTO:</td>
<td><input type="text" name="fechas_vencimiento" value="<?php echo $datos['fechas_vencimiento']; ?>" size="70"></td>
<tr valign="baseline">
<td nowrap align="left">RAYA:</td>
<td><input type="text" name="porcentaje_vendedor" value="<?php echo $datos['porcentaje_vendedor']; ?>" size="70"></td>
</tr>
