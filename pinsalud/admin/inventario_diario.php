...
$mostrar_datos_sql = "SELECT * FROM ventas WHERE fecha_anyo = '$fecha' ORDER BY cod_ventas DESC";
...
WHERE fecha_anyo = '$fecha' AND tipo_pago ='1'";
...
sum(vlr_total_compra) As vlr_total_compra, Sum(((precio_venta/1.16)*(iva/100))*unidades_vendidas) As sum_iva FROM ventas 
WHERE fecha_anyo = '$fecha' AND tipo_pago ='2'";
...
$egresos = "SELECT Sum(costo) As total_egreso FROM egresos WHERE fecha_anyo = '$fecha'";
...
$sum_abonos_valor = "SELECT Sum(abonado) AS abonado FROM cuentas_cobrar_abonos WHERE fecha_anyo = '$fecha'";
...
$sql_prod_fiados = "SELECT Sum(vlr_total_venta-(vlr_total_venta*(descuento_ptj/100))) As vlr_total_venta FROM productos_fiados WHERE fecha_anyo = '$fecha'";
...
if ($fecha <> NULL) {?>
<center>
<fieldset><legend><font color='yellow' size='+3'>VENTAS TOTALES DIA: <?php echo $fecha.' - '.$hora;?></font></legend>
<table width='100%'>
<br>
<tr>
...
<br>
<fieldset><legend><font color='yellow' size='+3'>VENTAS DIA: <?php echo $fecha.' - '.$hora;?></font></legend>
<table width='100%'>
<tr>
