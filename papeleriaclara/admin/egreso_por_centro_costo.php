...
$nombre_ccosto = mysql_escape_string($_GET['nombre_ccosto']);
$mostrar_datos_sql = "SELECT * FROM egresos WHERE nombre_ccosto = '$nombre_ccosto' order by fecha_invert DESC";
$consulta = mysql_query($mostrar_datos_sql, $conectar) or die(mysql_error());
$datos = mysql_fetch_assoc($consulta);

$datos_sql = "SELECT SUM(costo) AS costo FROM egresos WHERE nombre_ccosto = '$nombre_ccosto' order by fecha_invert DESC";
...
<td><select name="nombre_ccosto" require autofocus>
<?php $sql_consulta="SELECT DISTINCT nombre_ccosto FROM egresos ORDER BY nombre_ccosto DESC";
$resultado = mysql_query($sql_consulta, $conectar) or die(mysql_error());
while ($contenedor=mysql_fetch_array($resultado)) {?>
<option style="font-size:20px" value="<?php echo $contenedor['nombre_ccosto'] ?>"><?php echo $contenedor['nombre_ccosto']?></option>
<?php }?></select></td>
<tr valign="baseline">
...
<?php
if (isset($_GET['nombre_ccosto'])) {
echo "<font color ='yellow' size='+3'>RESULTADOS PARA: ".$nombre_ccosto."</font>";
