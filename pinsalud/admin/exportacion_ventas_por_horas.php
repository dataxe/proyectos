...
<?php $sql_consulta="SELECT distinct fecha_anyo, fecha FROM ventas ORDER BY fecha DESC";
$resultado = mysql_query($sql_consulta, $conectar) or die(mysql_error());
while ($contenedor=mysql_fetch_array($resultado)) {?>
<option style="font-size:13px" value="<?php echo $contenedor['fecha'].'-'.$contenedor['fecha_anyo'] ?>"><?php echo $contenedor['fecha_anyo'] ?></option>
<?php }?></select></td>
</table>
<table>
