...
$(function () {
    $('#container').highcharts({
        ...
        title: {
                text: '<?php echo utf8_decode("UTILIDAD VS GASTOS POR AÑOS")?>'
        },
        subtitle: {
        ...
        xAxis: [{
            categories: [
<?php
$sql = "SELECT fecha FROM egresos GROUP BY anyo ORDER BY fecha ASC";
$result = mysql_query($sql, $conectar) or die(mysql_error());
$total = mysql_num_rows($result);
while ($registros = mysql_fetch_array($result)) {
$fecha_anyo = date('Y', $registros["fecha"])
?>
'<?php echo $fecha_anyo ?>',
<?php
}
?>
        ...
        series: [{
            ...
            data: [
<?php
$sql = "SELECT SUM(vlr_total_venta-vlr_total_compra) AS vlr_total_venta FROM ventas GROUP BY anyo ORDER BY fecha ASC";
$result = mysql_query($sql, $conectar) or die(mysql_error());
while ($registros = mysql_fetch_array($result)) {
?>
...
?>
            ...
            data: [
<?php
$sql = "SELECT SUM(costo) AS costo FROM egresos GROUP BY anyo ORDER BY fecha ASC";
$result = mysql_query($sql, $conectar) or die(mysql_error());
while ($registros = mysql_fetch_array($result)) {
?>
