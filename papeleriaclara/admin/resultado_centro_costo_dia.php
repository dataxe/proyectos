...
$fecha_anyo = $frag[0];
$nombre_ccosto = $frag[1];

$mostrar_datos_sum = "SELECT Sum(vlr_total_venta) As vlr_total_venta_sum, Sum(vlr_total_compra) As vlr_total_compra_sum, Sum(((precio_venta/1.16)*(iva/100))*unidades_vendidas) As sum_iva 
FROM ventas WHERE fecha_anyo = '$fecha_anyo' AND nombre_ccosto = '$nombre_ccosto' ORDER BY fecha_hora DESC";
...
$mostrar_datos_sql = "SELECT * FROM ventas WHERE fecha_anyo = '$fecha_anyo' AND nombre_ccosto = '$nombre_ccosto' ORDER BY fecha_hora DESC";
...
<table align="center">
<td nowrap align="right">DIA - CENTRO COSTO:</td>
<td bordercolor="0">
<select name="fecha">
<?php $sql_consulta1="SELECT DISTINCT fecha_anyo, nombre_ccosto FROM ventas ORDER BY fecha DESC";
$resultado = mysql_query($sql_consulta1, $conectar) or die(mysql_error());
while ($contenedor=mysql_fetch_array($resultado)) {?>
<option value="<?php echo $contenedor['fecha_anyo'].'-'.$contenedor['nombre_ccosto'] ?>"><?php echo $contenedor['fecha_anyo'].' - '.$contenedor['nombre_ccosto'] ?></option>
<?php }?>
</select></td></td>
<tr valign="baseline">
