...
$(function () {
    $('#container').highcharts({
        ...
        xAxis: {
            categories: [
<?php
$sql = "SELECT SUM(costo) AS costo, fecha_invert FROM egresos GROUP BY fecha_invert ORDER BY fecha_invert ASC";
$result = mysql_query($sql, $conectar) or die(mysql_error());
while ($registros = mysql_fetch_array($result)) {
$fecha_seg = strtotime($registros["fecha_invert"]);
$fecha = date("d/m/Y", $fecha_seg);
?>
'<?php echo $fecha?>',
<?php
}
?>
        ...
        series: [{
            ...
            data: [
<?php
$sql = "SELECT SUM(costo) AS costo, fecha_mes, fecha_anyo FROM egresos GROUP BY fecha_invert ORDER BY fecha_invert ASC";
$result = mysql_query($sql, $conectar) or die(mysql_error());
while ($registros = mysql_fetch_array($result)) {
?>
